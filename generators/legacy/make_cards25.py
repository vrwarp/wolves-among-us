...
def card(c,ox,g):
    ...
    for tier,n in CARDS[g]:
        ...
        if n=='Verse Order':
            ...
            c.setFillColor(GREY); c.setFont("Helvetica",10.5)
            c.drawString(x,y-33,f"Row {g} — all 7 doors. Copy BOTH letters at each.")
            for li,row in enumerate((DOORS[:4],DOORS[4:])):
                dy=y-56-li*22; xx=x
                for d in row:
                    c.setFillColor(black); c.setFont("Helvetica-Bold",11.5); c.drawString(xx,dy,d)
                    c.setStrokeColor(black); c.setLineWidth(1.1); c.setFillColor(white)
                    c.rect(xx+20,dy-4,36,18,stroke=1,fill=1); xx+=64
        else:
...
def build(path,order,note=None):
    ...
    for i in range(0,len(order),2):
        ...
        if note: c.drawString(20,8,note(i//2))
        c.showPage()
    c.save()
uniq=list(range(1,NG+1))
build("footprints-print-pack/01-index-cards/index-cards-25-unique.pdf",uniq+[None])
deck=uniq*3+uniq[:5]
build("footprints-print-pack/01-index-cards/index-cards-80-deck.pdf",deck,
      note=lambda p:f"deck {min(3,p//13+1)}" if p<39 else "spares")
c=canvas.Canvas("/dev/null",pagesize=(PW,PH))
worst=min(PH-M-(HEADER+COLH+sum(rh(c,g,n) for _,n in CARDS[g]))-DEAD_TOP for g in CARDS)
assert worst>0 and all(sum(PTS[n] for _,n in CARDS[g])==11 for g in CARDS)
print(f"25 unique cards -> 13 pages · 80-card deck -> {len(deck)//2} pages · min clearance {worst:.0f}pt")
