...
s=json.load(open('cardspec25.json'))
...
for d in DOORS:
    ...
    c.drawString(40,PH-46,f"{floor}  ·  AMONG US: FOOTPRINTS")
    c.setFillColor(black); c.setFont("Helvetica-Bold",92); c.drawString(36,PH-128,f"DOOR {d}")
    c.setFillColor(GREY); c.setFont("Helvetica-Bold",16)
    c.drawRightString(PW-40,PH-108,"Find YOUR GROUP NUMBER.")
    c.drawRightString(PW-40,PH-130,"Copy BOTH letters next to it.")
    c.setStrokeColor(black); c.setLineWidth(2.5); c.line(36,PH-146,PW-36,PH-146)
    top=PH-166; colw=(PW-76)/5; rowh=72
    for i in range(NG):
        col,r=i%5,i//5
        x=40+col*colw; y=top-r*rowh
        c.setStrokeColor(LINE); c.setLineWidth(.8); c.line(x,y-rowh+10,x+colw-16,y-rowh+10)
        c.setFillColor(GREY); c.setFont("Helvetica-Bold",8); c.drawString(x,y-13,"GROUP")
        c.setFillColor(black); c.setFont("Helvetica-Bold",26); c.drawString(x,y-40,str(i+1))
        c.setStrokeColor(black); c.setLineWidth(2); c.setFillColor(white)
        c.rect(x+colw-96,y-46,74,40,stroke=1,fill=1)
        c.setFillColor(black); c.setFont("Helvetica-Bold",30)
        c.drawCentredString(x+colw-59,y-37,CODE[d][str(i+1)])
...
for page in range((NG+per-1)//per):
    ...
    c.setFillColor(black); c.setFont("Helvetica-Bold",18); c.drawString(30,PH-38,"COUNSELOR ANSWER SHEET")
    c.setFillColor(GREY); c.setFont("Helvetica",9.5)
    c.drawString(30,PH-52,f"Groups {lo}–{hi}.  Find the group number on the student's card, then check their answers here.")
    c.setFillColor(black); c.setFont("Helvetica-Bold",9)
    c.drawRightString(PW-30,PH-36,"TONIGHT'S TARGET: ______ pts     PEW BIBLE EDITION: ___________________________")
    c.setFillColor(GREY); c.setFont("Helvetica",7.5)
    c.drawRightString(PW-30,PH-50,"Page numbers are worthless against a different printing.")
    c.setStrokeColor(black); c.setLineWidth(1.4); c.line(30,PH-60,PW-30,PH-60)
    W=(PW-60-3*9)/4; H=(PH-60-26-9-16)/2
    for i in range(hi-lo+1):
        block(c,30+(i%4)*(W+9), PH-60-14-(i//4)*(H+9)-H, W,H, lo+i)
    c.setFillColor(GREY); c.setFont("Helvetica",7.5)
    c.drawCentredString(PW/2,13,f"page {page+1} of {(NG+per-1)//per}   ·   all four counselors carry all pages")
    c.showPage()
...
for word,rest in SENT:
    ...
    c.drawRightString(PW-44,20,f"facilitator: this box holds the {word} cards")
    c.showPage()
