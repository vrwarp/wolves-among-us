...
class Doc:
    ...
    def title(s):
        ...
        c.drawString(M,s.y-62,"20–25 students, grades 6–12  ·  4 counselors  ·  2–3 rounds  ·  students never need a phone")
...
def build(total=None,path="/tmp/pb/playbook.pdf"):
    ...
    d.b("The round clock","counts DOWN from 8:00 of floor time. Only meetings pause it. A successful Sabotage adds 1:00; a failed one subtracts 1:30. There are no other timers — if no Sabotage fires, the round simply ends at 0:00.")
    ...
    d.b("Central Command — lobby desk.","Runs the round clock and every meeting. Marks the whiteboard. Verifies portable tasks downstairs. One end of the red-ball run. Runs Sabotage from its script page.")
    d.b("Foreman — stage.","Cups, apples, flight zone. Verify instantly, stamp, then make the student reset the station.")
    ...
    d.b("Ghost Guide — dead room.","Origami and quiet. One end of the red-ball run, handled at the doorway. Holds the phase stopwatch: meeting phases and the 2:00 Sabotage scramble.")
    d.b("Verification","is a stamp or marker — four unmistakably different, one per counselor, never set down. Your stamp is final; every checkable answer is on the 4-page answer sheet, by group number.")
    ...
    d.h1("Imposters")
    d.b("Blend in.","Do real tasks, earn real stamps — an imposter's card looks exactly like anyone else's, so “show me your card” proves nothing.")
    ...
    d.b("Sabotage safety.","Lights DIM, never off. One counselor stands at the stairs and does nothing else. Flashlights to the stairs and the Ghost Guide. Running during Sabotage costs a stamp, crossed out on the spot.")
    ...
    d.b("Reveal the result on the spot.","Say whether the ejected player was an imposter. Crewmate: tick the death board. Imposter: no tick. Then everyone back to the floor — Central Command never lets the clock stretch.")

    d.h1("Quick rulings")
    for a,b_ in [
      ("“Can I see your card?”","Allowed. Imposters carry real stamps, so it proves nothing."),
      ...
      ("A student runs","Verbal warning, then cross out one stamp box and initial it."),
      ("Argument over a stamp","The counselor's stamp is final. No appeals."),
    ...
    for i,t in enumerate([
      ...
      "“Find a counselor to stamp each task. Four counselors, four stamps.”",
      ...
      "“If the lights dim, freeze and listen for Central Command. One item per person.”"],1):
        d.b(f"{i}.",t)
    d.b("Privately, before the room fills, tell the 3 imposters:","do real tasks, get real stamps, count 20 between kills, nothing in the first minute, target anyone alone — and tapping a counselor triggers Sabotage.")
    ...
    for t in ["7 door sheets taped up, all four corners","6 Sabotage props taped at their printed doors",
      ...
      "Both stopwatches zeroed","A spoon in EVERY student's hand","Imposters briefed privately",
